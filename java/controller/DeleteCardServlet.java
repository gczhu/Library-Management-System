...
public class DeleteCardServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ApiResult res = library.removeCard(id);
        if (!res.ok) {
            this.getServletContext().setAttribute("msg", "该借书证有尚未归还的图书，删除失败");
            response.sendRedirect("TraversalCardServlet?method=error");
            return;
        } else {
            library.unbondCard(id);
        }
        // 释放数据库
        ...
        response.sendRedirect("TraversalCardServlet?method=list");
        return;
    }
