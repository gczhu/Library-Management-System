...
public class LibraryManagementSystemImpl implements LibraryManagementSystem {
    ...
    @Override
    public ApiResult newUser(User user) {
        ...
        try {
            ...
            if (res.next()) {     // 如果该账号已经存在，则报错
                throw new Exception("The account already exists.");
            } else {      // 否则，将该账户的信息写入到数据库当中
                String sql1 = "INSERT INTO users\n" +
                        "VALUES(?, ?, ?, ?)\n";
                pstmt = conn.prepareStatement(sql1);
                pstmt.setString(1, user.getAccount());
                pstmt.setString(2, user.getPassword());
                pstmt.setString(3, user.getRole().getStr());
                pstmt.setInt(4, user.getCardId());
                pstmt.executeUpdate();
            }
            commit(conn);
        } catch (Exception e) {
        ...
        return new ApiResult(true, "newUser(User user) success");
    }
    ...
    public User queryUser(String account) {
        ...
        try {
            ...
            ResultSet res = pstmt.executeQuery();
            if (!res.next()) {        // 如果该用户不存在，则报错
                throw new Exception("The user doesn't exists.");
            } else {        // 否则获取该用户的password和role，并以对象返回
            ...
            rollback(conn);
            return new User(null, null, User.UserRole.User, 0);
        }
        if (role.equals("U")) {
            return new User(account, password, User.UserRole.User, cardId);
        }
        return new User(account, password, User.UserRole.Admin, cardId);
    }

    @Override
    public ApiResult queryStock(int bookId) {
        Connection conn = connector.getConn();
        Integer stock = 0;
        try {
            conn.setAutoCommit(false);
            ...
            rollback(conn);
            return new ApiResult(false, e.getMessage(), stock);
        }
        return new ApiResult(true, "queryStock(int bookId) success", stock);
    }

    @Override
    public ApiResult queryCard(int cardId) {
        Connection conn = connector.getConn();
        ...
        return new ApiResult(true, "queryCard(int cardId) success", cardList);
    }

    @Override
    public int queryCard(String name, String department, String type) {
        Connection conn = connector.getConn();
        try {
            conn.setAutoCommit(false);
            PreparedStatement pstmt = null;
    ...
    @Override
    public ApiResult bondCard(String account, int cardId) {
        Connection conn = connector.getConn();
        try {
            conn.setAutoCommit(false);
            PreparedStatement pstmt = null;
        ...
        return new ApiResult(true, "bondCard(String account, int cardId) success");
    }

    @Override
    public ApiResult unbondCard(int cardId) {
        Connection conn = connector.getConn();
        try {
            conn.setAutoCommit(false);
            PreparedStatement pstmt = null;
        ...
        return new ApiResult(true, "unbondCard(String account) success");
    }
